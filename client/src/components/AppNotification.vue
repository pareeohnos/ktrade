...
<script lang="ts">
  ...
  const titleColours: { [key: string]: string } = {
    info: "text-blue-500",
    error: "text-red-500",
  ...
  export default defineComponent({
    ...
    setup({ notification }: { notification: Notification }) {
      const colourBoxClasses = computed(() => {
        let classes = ["flex", "items-center", "justify-center", "w-12"];
        classes.push(bgColours[notification.type]);

        return classes;
      });

      const titleClasses = computed(() => {
        return ["font-semibold", titleColours[notification.type]];
      });
